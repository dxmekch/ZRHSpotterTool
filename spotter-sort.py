...
if ( "arr" in sys.argv[1] ):
	...
	if os.path.exists("timetable.zrh.arrival.json"):
		os.remove("timetable.zrh.arrival.json")		# delete the outdated file
	webfile = open("timetable.zrh.arrival.json", "w")
elif ( "dep" in sys.argv[1] ):
	print ("Zurich Airport ZRH - Departures")
	if os.path.exists("timetable.zrh.departure.json"):
		os.remove("timetable.zrh.departure.json")
	webfile = open("timetable.zrh.departure.json", "w")
