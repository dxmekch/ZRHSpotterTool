        ...
        if(opts.tomorrow):
            if(opts.std):
                if(opts.arr):
                    ...
                    dump_to_json_file(arrival_standard_tomorrow,    outdir + 'timetable.arrival.tom.standard_' + str(int(opts.timeoffs)) + '.json')
                if(opts.dep):
                    ...
                    dump_to_json_file(departures_standard_tomorrow, outdir + 'timetable.departure.tom.standard_' + str(int(opts.timeoffs)) + '.json')
            if(opts.spt):
                if(opts.arr):
                    print('[+] downloading (spotter)  (arrivals)  (tomorrow) ...')
                    arrival_spotter_tomorrow     = zrh.fetch('Arrival', spotter=True, tomorrow=True)
                    dump_to_json_file(arrival_spotter_tomorrow,     outdir + 'timetable.arrival.tom.spotter_' + str(int(opts.timeoffs)) + '.json')
                if(opts.dep):
                    print('[+] downloading (spotter)  (departures) (tomorrow) ...')
                    departures_spotter_tomorrow  = zrh.fetch('Departure', spotter=True, tomorrow=True)
                    dump_to_json_file(departures_spotter_tomorrow,  outdir + 'timetable.departure.tom.spotter_' + str(int(opts.timeoffs)) + '.json')
